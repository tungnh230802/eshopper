...
using App.Models;
using App.Utilities;
using ASM_THIEN.Areas.Identity.Models.Account;
...
namespace App.Areas.Identity.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginOrRegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if ((!result.Succeeded))
                {
                    ...
                    if (user != null)
                    {
                        result = await _signInManager.PasswordSignInAsync(user.UserName, model.Login.Password, model.Login.RememberMe, lockoutOnFailure: false);
                    }
                }
        ...
        public async Task<IActionResult> Register(LoginOrRegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new AppUser { UserName = model.Register.UserName, Email = model.Register.Email };
